...
- `tests/cabinet/test_admin_user_reminders.py` — Python-модуль
  Классы: нет
  Функции: `test_permissions_are_registered`, `test_payload_validation`, `test_crud_flow_and_builtin_protection`, `test_audience_counts_per_channel`, `test_test_send_goes_to_the_admin`, `test_malformed_stored_conditions_do_not_break_reads` — Строка с некорректными conditions/текстами всё ещё отображается в списке и по id.
